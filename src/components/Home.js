...
import { DeleteTransaction, GetTransactions } from "./ApiCalls";
...
function Home(){
  ...
  // other transactions HOOKS for search
...
//---------------- GET TRANSACTIONS
  const fetchApiTransactions = async () => {
    const {success, dataTransactions} = await GetTransactions();
    if(success){
      setExistT(true);
...
const deleteTransaction = async () => { 
  handleClose();
let success = await DeleteTransaction(idTransaction);
if(success)
{
